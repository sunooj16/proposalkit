...
def block_text(block_id, title, body, source, line_no):
    """후보 블록 파일 내용.
...
def facts_yaml(candidates, source):
    lines = [
...
def build(text, source):
    ...
    blocks = [
        (f"{block_id}.md", block_text(block_id, title, body, source, line_of[block_id]))
        for block_id, title, body in sections
    ]
    return blocks, facts_yaml(collect_facts(sections), source), tags_txt(sections, source)
...
def run(args):
    ...
    blocks, facts, tags = build(source.read_text(encoding="utf-8"), source.as_posix())
